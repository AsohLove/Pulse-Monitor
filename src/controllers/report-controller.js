...
export async function getSingleMonitorChecks(req, res, next) {
  try {
    const monitor = await monitors.findSingleMonitorById(
      req.user.sub,
...
export async function downloadChecksCsv(req, res, next) {
  try {
    ...
    res.type('text/csv').attachment(`monitor-${monitor.id}.csv`).send(csv);
  } catch (err) {
    next(err);
  }
}
...
export async function getMonitorUptime(req, res, next) {
  try {
    ...
    const uptime =
      stats.total == 0
        ? 100
        : (Number(stats.successful) / Number(stats.total)) * 100;
